...
A reusable Codex skill for turning Figma frames, screenshots, visual specifications, and existing UI designs into high-fidelity, performance-aware React prototypes, using TypeScript and Tailwind CSS by default for new or unconstrained implementations.
...
Asset handling: detect downloaded formats from content and correct SVG/PNG suffix mismatches
Performance: remove avoidable waterfalls, defer heavy off-path modules, and avoid speculative memoization
...
- Detect persisted asset formats from content and repair SVG/PNG suffix mismatches before importing them.
- Review performance by impact: waterfalls, initial bundle, server/client boundaries, duplicate client work, re-renders, then rendering hot paths.
...
一个可复用的 Codex skill，用于将 Figma 画框、截图、视觉规范和已有 UI 设计转化为高保真且兼顾性能的 React prototype，并在新项目或未指定实现方式时默认使用 TypeScript 和 Tailwind CSS。
...
资源处理：根据下载内容识别真实格式，并修正 SVG/PNG 后缀不匹配问题
性能处理：消除可避免的瀑布式请求，延迟加载非首屏重型模块，并避免无依据的 memoization
...
- 根据文件内容识别持久化资源的格式，并在导入前修正 SVG/PNG 后缀不匹配问题。
- 按影响检查性能：瀑布式请求、初始 bundle、server/client 边界、重复的客户端工作、re-render，再到渲染热路径。
...
Figmaフレーム、スクリーンショット、ビジュアル仕様、既存のUIデザインを高忠実度でパフォーマンスに配慮したReactプロトタイプへ変換し、新規または実装方法が未指定の場合にTypeScriptとTailwind CSSを標準使用する、再利用可能なCodex skillです。
...
アセット処理：ダウンロード内容から実形式を判定し、SVG/PNGの拡張子不一致を修正
性能対応：不要なウォーターフォールを解消し、初期表示外の重いモジュールを遅延し、根拠のないmemoizationを避ける
...
- 保存したアセットの形式を内容から判定し、import前にSVG/PNGの拡張子不一致を修正します。
- ウォーターフォール、初期bundle、server/client境界、重複client処理、re-render、描画hot pathの順で性能を確認します。
